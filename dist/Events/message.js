module.exports = {
    ...
    run: (client, msg) => {
        ...
        prefixes.forEach(async (inp) => {
            ...
            if (!prefix)
                return;
            let args = msg.content.replace(prefix, "").split(" ").slice(1), input = msg.content.replace(prefix, "").split(" ")[0].toLowerCase(), cmd = client.commands.get(input) || client.aliases.get(input), member = client.guilds.cache.get(client.config.main_guild).member(msg.author.id);
            if (!cmd || (cmd.config.permissions.developer && (!member || (!msg.member._roles.includes(client.config.roles.lead_developer) && !msg.member._roles.includes(client.config.roles.developer)))))
                return;
            try {
