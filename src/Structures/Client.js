...
module.exports = class Bot extends Client {
  ...
  getUser (query) {
    const target = this.users.cache.get(query) || this.users.filter(u => u.username.toLowerCase().includes(query.toLowerCase())).first() || this.users.filter(u => u.tag.toLowerCase().includes(query.toLowerCase())).first()
    return target;
  };
